...
# FROM: testcases/service/app_device_get_device_type/app_device_get_device_type.yml
...
class TestCaseAppDeviceGetDeviceType(HttpRunner):
    ...
    config = Config("app通过设备UUID查询设备类型").base_url("${ENV(api_url)}")

    teststeps = [
        Step(RunTestCase("获取登录token").call(AppUserLogin).export(*["id", "token"])),
        Step(
            RunRequest("app通过设备UUID查询设备类型")
            .post("/app/device/get_device_type")
            .with_headers(**{"Content-Type": "application/json"})
            .with_json({"token": "$token", "uuid": "$uuid"})
            .validate()
...
if __name__ == "__main__":
    TestCaseAppDeviceGetDeviceType().test_start()
