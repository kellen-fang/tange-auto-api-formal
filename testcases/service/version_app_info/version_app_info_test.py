...
# FROM: testcases/service/version_app_info/version_app_info.yml
...
class TestCaseVersionAppInfo(HttpRunner):
    ...
    config = Config("app/device/list/least").base_url("${ENV(api_url)}")

    teststeps = [
        Step(RunTestCase("获取登录token").call(AppUserLogin).export(*["id", "token"])),
        Step(
            RunRequest("app版本升级")
            .post("version/app_info")
...
if __name__ == "__main__":
    TestCaseVersionAppInfo().test_start()
