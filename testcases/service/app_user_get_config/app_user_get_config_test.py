...
# FROM: testcases/service/app_user_get_config/app_user_get_config.yml
...
class TestCaseAppUserGetConfig(HttpRunner):
        ...
        Parameters(
            {
                ...
                "status_code-ok-status-code-ids-token-addDeviceVersion": [
                    [200, 1, "ok", 200, 0, "_app", "v1"]
                ],
            }
        ),
    )
    ...
    config = Config("tange-iCam365-用户拉取配置接").base_url("${ENV(api_url)}")

    teststeps = [
        Step(RunTestCase("获取登录token").call(AppUserLogin).export(*["id", "token"])),
        Step(
            RunRequest("用户拉取配置接")
            .post("app/user/get_config")
            ...
            .assert_equal("body.data.addDeviceVersion", "$addDeviceVersion")
        ),
    ]


if __name__ == "__main__":
    TestCaseAppUserGetConfig().test_start()
