...
# FROM: testcases/service/app_service_free_storage/app_service_free_storage.yml
...
class TestCaseAppServiceFreeStorage(HttpRunner):
    ...
    config = Config("免费领取云录像").base_url("${ENV(api_url)}")

    teststeps = [
        Step(RunTestCase("获取登录token").call(AppUserLogin).export(*["id", "token"])),
        Step(
            RunRequest("免费领取云录像")
            .post("http://static-cn01.tange365.com/app/service/free/storage")
...
if __name__ == "__main__":
    TestCaseAppServiceFreeStorage().test_start()
